...
class MazeApp:
    ...
    def setup_ui(self):
        ...
        input_frame.grid_columnconfigure(tuple(range(8)), weight=1)
        ...
        ttk.Label(input_frame, text="Delay (ms):").grid(row=0, column=4, padx=5)
        ...
        speed_spinbox.grid(row=0, column=5, padx=5)
        
        # Buttons
        ttk.Button(input_frame, text="Generate Maze", command=self.generate_maze).grid(row=0, column=6, padx=5)
        ttk.Button(input_frame, text="Solve Maze", command=self.solve_maze).grid(row=0, column=7, padx=5)
    ...
    def solve_maze(self):
        ...
        for y in range(len(self.grid)):
            for x in range(len(self.grid)):
                if self.grid[y][x] in [2, 3]:  # Clear visited and path cells
        ...
        end_time = time.time()
        print(f"Solved using {algorithm} in {end_time - start_time:.4f} seconds")
    ...
    def animate_solution(self, path):
        ...
        def draw_path_segment(index):
            ...
            if index + 1 < len(path):
                # Draw connecting path through walls
